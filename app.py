...
# =====================================================================
# РАЗДЕЛ 3. ФУНКЦИЯ ВЗАИМОДЕЙСТВИЯ С ИИ (БЕЗОПАСНЫЙ И СТАБИЛЬНЫЙ ВЫЗОВ)
# =====================================================================
def generate_perfect_math(selected_class, subject, section, objectives_list, w_type, var_qty, t_count, score):
    ...
    if w_type == "Формативное оценивание (ФО)":
        format_instructions = f"Составь краткие экспресс-карточки ФО. Обязательно добавь блок самооценивания ученика и краткие ответы для учителя."
    elif w_type == "СОР":
        format_instructions = f"Составь полноценный СОР. Обязательно добавь HTML-таблицу 'Спецификация и дескрипторы' со столбцами: № задания, Проверяемая ЦО, Дескрипторы, Балл. Добавь эталонное решение."
    else:
        format_instructions = f"Составь комплексную итоговую работу СОЧ, разделенную на Блок А, Блок B и Блок C. Обязательно добавь HTML-таблицу 'Рубрика для выставления баллов' со столбцами: № задания, Проверяемая ЦО, Критерии, Балл. Добавь схему ответов."

    prompt = f"Ты опытный учитель-исследователь математики в Казахстане. Составь варианты {w_type} по предмету {subject} для {selected_class}. Раздел: {section}. Цели обучения ГОСО РК: {cos_string}. Количество вариантов: {var_qty}, заданий в каждом: {t_count}, макс. балл: {score}. {format_instructions} ВАЖНО: Оберни каждый вариант в HTML-тег <div class='vzaimo-card'>. Не используй разметку ```html, выводи только чистый HTML код, готовый для отображения на сайте."

    # Принудительно очищаем URL от любых скрытых символов переноса строк и пробелов
    clean_url = "[https://text.pollinations.ai](https://text.pollinations.ai)".strip()
    
    # Формируем параметры в JSON, чтобы избежать ошибок InvalidSchema и connection adapters
    payload = {
    ...
    # Безопасный POST запрос (символы промпта не ломают URL-строку)
    res = requests.post(clean_url, json=payload, headers=headers, timeout=60)
    
    if res.status_code == 200:
        return res.text
    else:
        # Резервный ультра-простой текстовый GET запрос без json-оберток
        fallback_url = f"[https://text.pollinations.ai/](https://text.pollinations.ai/){requests.utils.quote(prompt)}"
        res_fallback = requests.get(fallback_url.strip(), timeout=60)
        if res_fallback.status_code == 200:
            return res_fallback.text
        raise Exception(f"Ошибка ИИ-сервера. Статус: {res.status_code}")    
# =====================================================================
