...
def generate_perfect_math(selected_class, subject, section, objectives_list, w_type, var_qty, t_count, score):
    """
    Генерация учебных материалов с проверкой на ошибки
    """
    # Проверка на пустой список целей
    if not objectives_list:
        return """
        <div class="vzaimo-card" style="border-color: red; background-color: #fff0f0;">
            <h3 style="color: red;">⚠️ Ошибка: Не выбраны цели обучения!</h3>
            <p>Пожалуйста, вернитесь в боковую панель и выберите хотя бы одну цель обучения (ЦО).</p>
        </div>
        """
    ...
    try:
        ...
        for v in range(1, var_qty + 1):
            ...
            if w_type == "Формативное оценивание (ФО)":
                for i in range(1, t_count + 1):
                    ...
                    <li>
                        <b>Задание №{i}:</b> 
                        Вычислите или преобразуйте выражение, используя правила темы.
                        <br><small>Оценивается в {base_score_per_task} баллов</small>
                    </li>
                    """
                
                generated_html += f"""
                <div class="vzaimo-card">
                    <h2>🎯 Формативное оценивание</h2>
                    <div class="spec-block">
                    ...
                    <div class="answers-block">
                        ...
                        <p>Каждое задание оценивается по дескрипторам.</p>
                    </div>
                </div>
                """
            ...
            # --- СОР ---
                ...
                for i in range(1, t_count + 1):
                    ...
                    <li>
                        <b>Задание {i}:</b>
                        Решите задачу с полным обоснованием и проверкой.
                        <br><small>Балл: {base_score_per_task}</small>
                    </li>
                    """
                    table_rows += f"""
                    <tr>
            ...
            # --- СОЧ ---
                ...
                for i in range(1, t_count + 1):
                    ...
                    tasks_html += f"""
                    <li>
                        <b>Задание {i} ({level} уровень):</b>
                        Комплексная задача по теме с несколькими этапами решения.
                        <br><small>Балл: {base_score_per_task}</small>
                    </li>
                    """
                    table_rows += f"""
        ...
        return f"""
        <div class="vzaimo-card" style="border-color: red; background-color: #fff0f0;">
            <h3 style="color: red;">⚠️ Ошибка генерации: {str(e)}</h3>
            <p>Пожалуйста, проверьте параметры и попробуйте снова.</p>
        </div>
        """

# =====================================================================
# РАЗДЕЛ 4. ОСНОВНОЙ ЭКРАН
# =====================================================================
...
# Кнопка генерации
if st.sidebar.button("🚀 Сгенерировать бланки заданий", type="primary", use_container_width=True):
    if not selected_objectives:
        st.warning("⚠️ Пожалуйста, выберите хотя бы одну цель обучения (ЦО) в боковой панели!")
    else:
        with st.spinner("🔄 Генерация учебных материалов... Пожалуйста, подождите."):
            try:
                ...
                st.balloons()  # Небольшой праздничный эффект
            except Exception as e:
                st.error(f"❌ Произошла ошибка: {e}")

# =====================================================================
# РАЗДЕЛ 5. ВЫВОД И СТИЛИЗАЦИЯ
# =====================================================================
st.markdown(
    """
    <style>
...
if "generated_text" in st.session_state and st.session_state.generated_text:
    st.markdown(st.session_state.generated_text, unsafe_allow_html=True)
    st.info("💡 Нажмите **Ctrl + P**, чтобы отправить на печать или сохранить в PDF.")
else:
    st.info("👈 Настройте параметры в боковой панели и нажмите кнопку 'Сгенерировать бланки заданий'")
    
    # Показываем пример
    with st.expander("📖 Как это работает?"):
        st.markdown("""
        1. **Выберите класс** и направление (ЕМН/ОГН)
        2. **Выберите предмет** (математика, алгебра, геометрия)
        3. **Выберите раздел** из КТП
        4. **Отметьте цели обучения** (ЦО), которые нужно проверить
        5. **Выберите тип работы** (ФО, СОР, СОЧ)
        6. **Настройте параметры** (количество вариантов, заданий, баллов)
        7. **Нажмите "Сгенерировать"** - получите готовые материалы
        8. **Распечатайте** (Ctrl+P) или сохраните в PDF
        """)
