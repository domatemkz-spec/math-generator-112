...
# =====================================================================
# РАЗДЕЛ 3. ФУНКЦИЯ ВЗАИМОДЕЙСТВИЯ С ИИ (ДИФФЕРЕНЦИРОВАННЫЕ ФОРМАТЫ КАРТОЧЕК)
# =====================================================================
def generate_perfect_math(selected_class, subject, section, objectives_list, w_type, var_qty, t_count, score):
    ...
    Количество генерируемых вариантов: {var_qty}
    Количество заданий в одном варианте: {t_count}
    Общий балл за всю работу: {score}
    
    {format_instructions}
    
    ВАЖНО: Никакого лишнего текста или системных комментариев (типа ```html) до и после HTML-кода. Начни сразу с тега <div class="vzaimo-card">. Задания должны содержать корректные, решаемые математические данные без неопределенностей.
    """
    ...
    if response.status_code == 200:
        ...
        alt_response = requests.get(f"[https://text.pollinations.ai/](https://text.pollinations.ai/){requests.utils.quote(prompt)}")
        if alt_response.status_code == 200:
            return alt_response.text
        raise Exception(f"Все шлюзы ИИ временно перегружены. Попробуйте еще раз через минуту.")
# =====================================================================
