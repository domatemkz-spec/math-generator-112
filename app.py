...
# =====================================================================
# РАЗДЕЛ 3. АВТОНОМНЫЙ МЕТОДИЧЕСКИЙ ГЕНЕРАТОР БЛАНКОВ (БЕЗ СЕТЕВЫХ СБОЕВ)
# =====================================================================
def generate_perfect_math(selected_class, subject, section, objectives_list, w_type, var_qty, t_count, score):
    # Формируем список целей для отображения в спецификации
    cos_string = "".join([f"<li>{co}</li>" for co in objectives_list])
    
    # Распределяем баллы по заданиям (округление)
    base_score_per_task = max(1, score // t_count)
    
    generated_html = ""
    
    # Генерируем нужное количество вариантов
    for v in range(1, var_qty + 1):
        
        # 1. ФОРМАТ КАРТОЧЕК ДЛЯ ФОРМАТИВНОГО ОЦЕНИВАНИЯ (ФО)
        if w_type == "Формативное оценивание (ФО)":
            tasks_html = ""
            for i in range(1, t_count + 1):
                tasks_html += f"<li><b>Задание №{i}:</b> Вычислите или преобразуйте выражение, используя правила текущей темы/ЦО. Проверьте полученный результат. (Оценивается в {base_score_per_task} б.)</li>"
                
            generated_html += f"""
            <div class="vzaimo-card">
                <h2>🎯 ФО — {subject} ({selected_class})</h2>
                <div class="spec-block">
                    <b>Контекст/Тема:</b> {section}<br>
                    <b>Проверяемые цели обучения:</b>
                    <ul>{cos_string}</ul>
                </div>
                <h3>ВАРИАНТ №{v}</h3>
                <p><i>Инструкция: Внимательно прочитайте и выполните задания. Максимальный балл: {score}.</i></p>
                <ol class="tasks-list">
                    {tasks_html}
                </ol>
                ...
                <div class="answers-block">
                    ...
                    <p>□ Понимаю и могу объяснить тему | □ Понимаю, но сомневаюсь в вычислениях | □ Требуется помощь учителя</p>
                    <hr style="border: 1px dashed #f97316; margin: 10px 0;">
                    <b>Краткий гид для взаимопроверки:</b> Конечные числовые значения должны строго соответствовать математическим тождествам проверяемых ЦО.
                </div>
            </div>
            """
            
        # 2. ФОРМАТ КАРТОЧЕК ДЛЯ СУММАТИВНОГО ОЦЕНИВАНИЯ ЗА РАЗДЕЛ (СОР)
        elif w_type == "СОР":
            tasks_html = ""
            table_rows = ""
            for i in range(1, t_count + 1):
                tasks_html += f"<li>Выполните подробное пошаговое аналитическое решение для математической модели по теме: '{section}'. Покажите все этапы тождественных преобразований. (Балл: {base_score_per_task})</li>"
                table_rows += f"""
                <tr>
                    <td>{i}</td>
                    <td>Выбранная ЦО ГОСО</td>
                    <td>- Правильно записывает математическое условие и выбирает формулу;<br>- Выполняет тождественные вычисления без арифметических ошибок;<br>- Находит и записывает окончательный ответ.</td>
                    <td>{base_score_per_task}</td>
                </tr>
                """
            ...
            <div class="vzaimo-card">
                ...
                <div class="spec-block">
                    ...
                    <b>Проверяемые цели обучения (ГОСО РК):</b>
                    <ul>{cos_string}</ul>
                ...
                <h3>📋 Спецификация и дескрипторы оценивания (Вариант №{v})</h3>
                <table>
                    <thead>
                        <tr>
                            <th>№ задания</th>
                            <th>Проверяемая цель (ЦО)</th>
                            <th>Дескрипторы (Пошаговый ход решения)</th>
                            <th>Балл</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div class="answers-block">
                    <b class="answers-block-title">Критерии ответов для модерации:</b>
                    <p>Проверка осуществляется строго по дескриптору. При наличии альтернативного верного математического пути решения, баллы выставляются в полном объеме согласно регламенту критериального оценивания РК.</p>
                </div>
            </div>
            """
            
        # 3. ФОРМАТ КАРТОЧЕК ДЛЯ СУММАТИВНОГО ОЦЕНИВАНИЯ ЗА ЧЕТВЕРТЬ (СОЧ)
        else:
            tasks_html = ""
            table_rows = ""
            # Делим задания СОЧ на логические блоки уровней А, B, C
            for i in range(1, t_count + 1):
                if i <= max(1, t_count // 3):
                    level = "Блок А (Базовые знания и понимание)"
                elif i <= max(2, (t_count * 2) // 3):
                    level = "Блок B (Применение алгоритмов)"
                else:
                    level = "Блок C (Сложные задачи / Математическое моделирование)"
                    
                tasks_html += f"<li><b>{level}:</b> Решите комплексную многоэтапную задачу, опираясь на свойства и теоремы из раздела '{section}'. Обоснуйте каждый логический шаг. (Балл: {base_score_per_task})</li>"
                table_rows += f"""
                <tr>
                    <td>{i}</td>
                    <td>Целевой стандарт</td>
                    <td>{level}</td>
                    <td>- Демонстрирует глубокое понимание структуры задачи;<br>- Безошибочно реализует вычислительный алгоритм;<br>- Проводит логическое обоснование и анализ ответа.</td>
                    <td>{base_score_per_task}</td>
                </tr>
                """
            ...
            <div class="vzaimo-card">
                ...
                <div class="spec-block">
                    <b>Итоговое суммативное оценивание за четверть:</b> {section}<br>
                    <b>Охваченные стандарты обучения:</b>
                    <ul>{cos_string}</ul>
                    <b>Рекомендуемое время выполнения:</b> 40-45 минут | <b>Максимальный балл:</b> {score}
                </div>
                <h3>ВАРИАНТ №{v}</h3>
                <ol class="tasks-list">
                    {tasks_html}
                ...
                <h3>📐 Рубрика для выставления баллов (Вариант №{v})</h3>
                <table>
                    <thead>
                        <tr>
                            <th>№ задания</th>
                ...
                <div class="answers-block">
                    <b class="answers-block-title">Официальная схема выставления баллов (Ключи схемы):</b>
                    <p>Каждый шаг дескриптора оценивается дискретно в 1 балл. Выставление дробных баллов не допускается.</p>
                </div>
            </div>
            """
