...
# =====================================================================
# РАЗДЕЛ 3. ФУНКЦИЯ ВЗАИМОДЕЙСТВИЯ С ИИ ПО ВЕБ-API БЕЗ СБОЙНЫХ БИБЛИОТЕК
# =====================================================================
def generate_perfect_math(selected_class, subject, section, objectives_list, w_type, var_qty, t_count, score):
    ...
    # Делаем полностью автономный веб-запрос к бесплатному открытому ИИ-узлу
    response = requests.post(url, headers=headers, data=json.dumps(payload))
    if response.status_code == 200:
        res_json = response.json()
        return res_json['choices'][0]['message']['content']
    else:
        raise Exception(f"Ошибка удаленного сервера: статус {response.status_code}")
